    ...
    beforeEach(() => {
        ...
        describe('drag and drop test', () => {
            it("should test bun drop and it exists", () => {
                ...
                cy.get('ul[class*=IngredientSection_list] a').first().contains("булка").trigger('dragstart', { dataTransfer, force: true }) //если тестить без интернета, то ругается на отсутствии загруженной картинки
                cy.get('[class*=BurgerConstructor_item]:first').trigger('drop')
                cy.get('[class*=BurgerConstructor_item]:first').find('[class*=BurgerConstructor_top_bun]').should('exist')
            })
            it("should test that bun cant be dropped to container for ingredient", () => {
                const dataTransfer = new DataTransfer();
                cy.get('ul[class*=IngredientSection_list] a').first().contains("булка").trigger('dragstart', { dataTransfer, force: true }) //если тестить без интернета, то ругается на отсутствии загруженной картинки
                cy.get('[class*=BurgerConstructor_chosen_items]').trigger('drop')
            ...
            it("should test ingredient drop and existing", () => {
                ...
                cy.get('[class*=BurgerConstructor_chosen_items]').trigger('drop')
                cy.get('[class*=BurgerConstructor_chosen_items]').find('[class*=EmptyCard_wrapper]').should('not.exist')
            })
        })
        describe('order sending test', () => {
            it('should test order sending', () => {
                cy.intercept('POST', "api/orders", {fixture: "order.json"})
                cy.get('ul[class*=IngredientSection_list] a').first().contains("булка").trigger('dragstart')
                cy.get('[class*=BurgerConstructor_item]:first').trigger('drop')
                cy.get('ul[class*=IngredientSection_list] a').eq(2).trigger('dragstart')
                cy.get('[class*=BurgerConstructor_chosen_items]').trigger('drop')
